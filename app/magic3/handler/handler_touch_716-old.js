...
var TC = {
	...
	init: function() {
		...
		//if (TC.dbg) console.log("tp start, tp,this.st:",tp,this.st);

		if (ew.temp.bar) {
			if (180 < (((tp[5] & 0x0F) << 8) | tp[6])) {
				if (TC.dbg) print("touch input in bar:", tp);
				...
				if (tp[2]) {
					...
					if (this.x != tp[4]) {
						...
						if (this.rp) {
							...
							if (TC.val.reverce) side = 1 - side;
							side ? TC.val.cur++ : TC.val.cur--;
							if (TC.val.up < TC.val.cur) TC.val.cur = TC.val.up;
							else if (TC.val.cur < TC.val.dn) TC.val.cur = TC.val.dn;
							TC.emit("bar", side ? 1 : -1, TC.val.cur);
						}
					}
				}
				else {
				...
				if (TC.dbg) console.log("in bar return, tp:",tp);
				return;
			}// else if (!this.do) this.st = 1;
			
		} else if (this.do) this.st = 0;
		if (tp[3] === 0 || tp[3] === 128) {
			if (TC.dbg) print("touch input in main:", tp);
			if (tp[2] == 1 && this.st) {
			...
			if (this.do && getTime() - this.time > 1 && tp[2] == 1) {
				...
				//print(a,this.aLast)
				if (a != 0 && this.aLast != a) {
					if (TC.dbg) console.log("in main, gesture:",a);
					this.aLast = a;
					this.do = 0;
					//this.st=0;
