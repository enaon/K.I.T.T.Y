	...
	init: function() {
		...
		if (ew.temp.bar) {
			if (180 < (((tp[5] & 0x0F) << 8) | tp[6])) {
				if (this.dbg) print("touch input in bar:", tp);
				...
				if (this.dbg) console.log("in bar return, tp:", tp);
				return;
		...
		if (tp[3] === 0 || tp[3] === 128) {
			if (this.dbg) print("touch input in main:", tp);
			if (tp[2] == 1 && this.st) {
			...
			if (this.do && getTime() - this.time > 1 && tp[2] == 1) {
				...
				//print(a,this.aLast)
				if (a != 0 && this.aLast != a) {
					if (this.dbg) console.log("in main, gesture:", a);
					this.aLast = a;
					this.do = 0;
					//this.st=0;
