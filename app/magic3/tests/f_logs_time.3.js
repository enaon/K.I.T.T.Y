...
ew.UI.nav.back.replaceWith(() => {
    ...
    if (ew.face[0].data.name != "month" && ew.face[0].data.name != "year")
        ew.face.go('f_logs_time', 0, { source: "month", key: ew.face[0].data.month + 1, lowL: "0", hiL: "50", style: 1, loop: 1 });
...
*/
        ...
        ew.UI.c.main._headerL = (i) => {
            ew.sys.buzz.nav(ew.sys.buzz.type.ok);
            if (i == 4) {
               // ew.UI.btn.c2l("main", "_headerL", 4, ew.face[0].data.month, "", 15, 6);
              //  ew.UI.btn.c2l("main", "_headerL", 5, ew.face[0].data.year, "", 15, 0);

                ew.face.go('f_logs_time', 0, { source: "month", key: this.data.month + 1, lowL: "0", hiL: "50", style: 1, loop: 1 });
            }
            else if (i == 5) {
               // ew.UI.btn.c2l("main", "_headerL", 5, ew.face[0].data.year, "", 15, 6);
               // ew.UI.btn.c2l("main", "_headerL", 4, ew.face[0].data.month, "", 15, 0);

                ew.face.go('f_logs_time', 0, { source: "year", key: this.data.year, lowL: "0", hiL: "50", style: 1, loop: 1 });
            }
        ...
        /*
                ...
                ew.UI.c.main._header = (i) => {
                    ...
                    if (i == 2) {
                        if (ew.face[0].data.name != "month")
                            ew.face.go('f_logs_time', 0, { source: "month", key: this.data.month +1, lowL: "0", hiL: "50", style: 1, loop: 1 });
                        else
                            ew.face.go('f_logs_time', 0, { source: "year", key: this.data.year, lowL: "0", hiL: "50", style: 1, loop: 1 });
                    }
                    else if (o.source == "year") {
                        if (Object.keys(logger.getStats("year", ew.face[0].data.year + (i == 1 ? -1 : 1))).length == 0) {
                            ew.UI.btn.ntfy(1, 1.5, 0, "_bar", 6, "NO LOGS FOR " + (ew.face[0].data.year + (i == 1 ? -1 : 1)), "", 15, 13);
                            g.flip();
                        }
                        else {
                            ew.face[0].data.year = ew.face[0].data.year + (i == 1 ? -1 : 1)
        ...
        ew.UI.c.main._main = (i) => {
            ...
            if (i == 6) {
                ...
                if (ew.face[0].data.year != Date().getFullYear() || ew.face[0].data.month != Date().getMonth()) {
                    /*let diffInDays = 0;
                    if (ew.face[0].data.month == Date().getMonth() - 1) {
                        let today = new Date(); // Σήμερα (10 Ιουλίου)
                        let previousDate = new Date(today.getFullYear(), ew.face[0].data.month, key); 
                        let diffInMs = today - previousDate; 
                        diffInDays = Math.floor(diffInMs / (1000 * 60 * 60 * 24));
                    }
                    if ( diffInDays && 30 <= diffInDays )  {
                    */
                    ew.sys.buzz.nav(ew.sys.buzz.type.na);
                ...
                if (ew.face[0].data.name != "day") {
                    if (ew.face[0].data.pos in ew.face[0].data.source)
                        key = ew.face[0].data.pos
                    else
                        key = ew.face[0].checkKeys(ew.face[0].data.source, ew.face[0].data.pos, -1, ew.face[0].data.loop);
                }
                else
                    key = ew.face[0].data.month + 1;


                ew.face.go('f_logs_time', 0, { source: ew.face[0].data.name == "month" ? "day" : "month", key: key, lowL: "0", hiL: "50", style: 1, loop: 1 });
            }
