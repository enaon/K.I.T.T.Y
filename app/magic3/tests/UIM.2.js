...
const UIM = (() => {
  ...
  const LAYOUTS = {
    ...
    bar:    { x:0, y:200, w:240, h:40, type:'row', slots:3 },
    full:   { x:0, y:0, w:240, h:240 }
  };
  ...
  const clearNotification = id => {
    ...
    if (notif.original) {
      // Διορθωμένο: χρήση βρόχου for αντί forEach
      for (let i = 0; i < notif.original.length; i++) {
        const comp = notif.original[i];
        if (comp) {
          // Δημιουργία νέου component αντί για απευθείας ανάθεση
          components[comp.id] = {
            id: comp.id,
            type: comp.type,
            region: comp.region,
          ...
          drawComponent(components[comp.id]);
        }
      }
    }
  };
    ...
    notify: (region, title, message, duration = 3) => {
      ...
      for (let i = 0; i < components.length; i++) {
        if (components[i] && components[i].region === region) {
          original.push({...components[i]}); // Δημιουργία αντιγράφου
          removeComponent(i);
        }
      }
...
const testUIM = () => {
  // 1. Αρχικοποίηση
  UIM.init({ primary: 0xF800, bg: 0x0000 });
  ...
  // 5. Notification μετά από 2 δευτερόλεπτα
  setTimeout(() => {
...
// ================== TOUCH SETUP ==================
const setupTestTouch = () => {
  setWatch(() => {
    // Προσομοίωση τυχαίας αφής
    const x = Math.random() * 240;
    const y = Math.random() * 240;
    UIM.handleTouch(x, y);
  }, BTN1, { repeat: true, debounce: 50 });
};
