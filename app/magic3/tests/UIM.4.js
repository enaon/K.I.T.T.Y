  ...
  const drawSlider = s => {
    ...
    if (s.mode === 'graph') {
      ...
      if (!s._drawn) {
        ...
        // Ζωγράφισε ΟΛΕΣ τις μπάρες με scaling
        for (let i = 0; i < barCount; i++) {
          const barValue = min + i;
          const isActive = (barValue <= value);
          const barHeight = (barValue / max) * s.h; // Scaling!
          const barX = s.x + (i * barWidth);
          const barY = s.y + s.h - barHeight;

          g.setColor(isActive ? s.activeColor : s.inactiveColor)
            .fillRect(barX + 1, barY, barX + barWidth - 1, s.y + s.h);
        }
      ...
      if (value < lastValue) {
        ...
        const oldBarHeight = (lastValue / max) * s.h;
        const oldBarX = s.x + (oldBarIdx * barWidth);
        const oldBarY = s.y + s.h - oldBarHeight;

        g.setColor(s.inactiveColor)
          .fillRect(oldBarX + 1, oldBarY, oldBarX + barWidth - 1, s.y + s.h);
      }

      // Β) ΑΝΑΝΕΩΣΗ ΝΕΑΣ ΜΠΑΡΑΣ
      const newBarIdx = value - min;
      const newBarHeight = (value / max) * s.h;
      const newBarX = s.x + (newBarIdx * barWidth);
      const newBarY = s.y + s.h - newBarHeight;

      g.setColor(s.activeColor)
        .fillRect(newBarX + 1, newBarY, newBarX + barWidth - 1, s.y + s.h);
      ...
      // Fill-mode (existing code)
      const ratio = (value - min) / (max - min);
      const fillWidth = s.w * ratio;
      g.setColor(s.bgColor || theme.secondary)
        .fillRect({ x: s.x, y: s.y, x2: s.x + s.w, y2: s.y + s.h, r: 10 });
      g.setColor(s.fillColor || theme.primary)
        .fillRect({ x: s.x, y: s.y, x2: s.x + fillWidth, y2: s.y + s.h, r: 10 });
      g.setColor(theme.text)
        .drawRect(s.x, s.y, s.x + s.w, s.y + s.h);
    }
  ...
  return {
    ...
    init: customTheme => {
      ...
      TC.on("bar", (a, b) => {
        ...
        // Update value based on a (-1/+1)
        const step = slider.step || 1;
        const newValue = slider.value + (a * step);
        UIM.update(slider.id, {
          value: Math.min(Math.max(newValue, slider.min), slider.max)
        });
    ...
    handleTouch: (x, y, l) => {
      ...
      for (let i = 0; i < components.length; i++) {
        ...
        // for (let i = components.length - 1; i >= 0; i--) {
          ...
          const step = c.step || 1;
          const newValue = c.value || 0;
          const clickedRatio = (x - c.x) / c.w;
          const targetValue = Math.round(c.min + clickedRatio * (c.max - c.min));

          // Calculate direction for single-step changes
          if (Math.abs(targetValue - newValue) <= step) {
            UIM.update(i, { value: targetValue });
          }
          else {
            const dir = (x < c.x + c.w / 2) ? -1 : 1;
            UIM.update(i, { value: Math.min(Math.max(newValue + (dir * step), c.min), c.max) });
          }

          if (c.onChange) c.onChange(components[i].value);
          return true;
        }
      }
      return false;
