Modules.addCached("OLED",function(){
	...
	exports.connectSPI = function(spi, dc,  rst, callback, options) {
		...
		var oled = {};
		gfxV=Graphics.createArrayBuffer(32,128,1,{vertical_byte : false,msb: false});
		gfxH=Graphics.createArrayBuffer(128,32,1,{vertical_byte : true, msb: false});
		oled.isOn=false;
	...
	ew.initOLED=function(rot,f){
			...
			ew.oled=require("OLED").connectSPI(spi,D28,D4,f,{cs:D29,rotation:rot});
			ew.oled.msg=(x,y,x1,y1)=>{
				if (ew.tid.oled) {clearTimeout(ew.tid.oled);ew.tid.oled=0;}
				if (!y) ew.tid.oled=setTimeout(()=>{ew.tid.oled=0;ew.oled.off();},5000);
				 ew.oled.gfx.setFont8x16();
 				 ew.oled.gfx.clear();
				 ew.oled.gfx.drawString(x,64-(ew.oled.gfx.stringWidth(x)/2),17);
				 ew.oled.flip();
        		 ew.oled.on();
        		 if (x1){
					if (ew.tid.oled) {clearTimeout(ew.tid.oled);ew.tid.oled=0;}
        			ew.tid.oled=setTimeout(()=>{ew.tid.oled=0;ew.oled.msg(x1,y1)},3000);
        		 }
			};
			setTimeout(()=>{
				global.g=ew.oled.gfx;
				ew.is.oled=1;
				ew.oled.msg("eucWatch v2");

			},2000);

	};
ew.initOLED(0); // init and set to Vertical mode
//ew.initOLED(270); // init and set to landscape mode
